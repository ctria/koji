...
def readTaggedDEBS(tag, package=None, arch=None, event=None,inherit=False,latest=True,owner=None,type=None):
    ...
    If type is not None, restrict the list to debs from builds of the given type.  Currently the
    ...
    fields = [('debinfo.name', 'name'),
              ...
              ('debinfo.id', 'id'),
              ('debinfo.size', 'size'),
              ('debinfo.payloadhash', 'payloadhash'),
    ...
    query = QueryProcessor(tables=tables, joins=joins, clauses=clauses,
                           columns=fields, aliases=aliases, values=data, transform=_fix_deb_row)
    ...
    def _iter_debs():
        for tagid in taglist:
            if tags_seen.has_key(tagid):
                ...
                #(Without this, we could report the same deb twice)
                continue
            else:
                tags_seen[tagid] = 1
...
def get_deb(debinfo, strict=False, multi=False):
    ...
    query = QueryProcessor(columns=[f[0] for f in fields], aliases=[f[1] for f in fields],
                           ...
                           values=data, transform=_fix_deb_row)
    if multi:
        return query.execute()
    ret = query.executeOne()
    if ret:
...
def list_debs(buildID=None, buildrootID=None, imageID=None, hostID=None, arches=None, queryOpts=None):
    ...
    query = QueryProcessor(columns=fields, aliases=aliases,
                           tables=['debinfo'], joins=joins, clauses=clauses,
                           values=locals(), transform=_fix_deb_row, opts=queryOpts)
    data = query.execute()
    return data
...
def import_deb(fn, buildinfo=None, brootid=None, wrapper=False, fileinfo=None):
    ...
    with open(fn, 'rb') as deb_file:
        debinfo['payloadhash'] = hashlib.sha1(deb_file.read()).hexdigest()
